"""LinkNet Wrapper for Foundation Models Comparison.

This module provides a wrapper around the trained LinkNet model to make it compatible
...
class LinkNetWrapper:
    """Wrapper for LinkNet model to segment nodules using 64×64 patches.
    
    This class handles:
    - Loading the trained LinkNet model
    - Extracting patches from full CT volumes
    - Running inference
    - Returning predictions in compatible format
    
    Args:
        checkpoint_path: Path to LinkNet model checkpoint
    ...
    def __init__(
        ...
        """Initialize LinkNet wrapper.
        
        Args:
            checkpoint_path: Path to saved LinkNet checkpoint
        ...
        print(f"✅ LinkNet loaded from {checkpoint_path}")
    ...
    def predict(
        ...
        Args:
            ...
            bbox: Bounding box (not used by LinkNet, for compatibility)
            src_voxel_origin: Voxel space origin
            src_world_matrix: World to voxel transformation matrix
            src_voxel_spacing: Voxel spacing
