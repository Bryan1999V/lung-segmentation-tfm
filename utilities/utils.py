...
def volume_transform(
    ...
    output_voxel_spacing: np.ndarray = None,
    **kwargs,
) -> np.ndarray:
...
def extract_patch(
    ...
    """Extract a patch from CT volume with optional augmentations and mask filtering.
    ...
    Args:
        ...
        translations: Float radius for random translations, or None
        coord_space_world: Whether coordinates are in world space
        mode: "2D" or "3D"
        model_name: Name of the model for channel determination
        mask_data: Optional mask data (not used anymore, kept for compatibility)

    Returns:
        Extracted and normalized patch
    """
    # NOTE: Mask filtering is now done AFTER extraction as weighted mask (30% background)
    # This preserves important context information for classification
    ...
    if translations is not None:
        translation_radius = np.random.uniform(0, translations)
        translation = sample_random_coordinate_on_sphere(translation_radius)
        coord = np.array(coord) + translation
    ...
    if coord_space_world:
        ...
        # Image coord sampling - just multiply by voxel spacing (DON'T add origin here)
    ...
    if mode == "2D":
        # Take the first (and should be only) slice for 2D mode
        patch = patch[0, :, :]
        
        # Special handling for masks
        if model_name == "mask":
            # Masks: no normalization, single channel
            patch = patch[np.newaxis, :, :]  # (1, H, W)
            return patch.astype(np.float32)
        
        # For images: DO NOT normalize here, will be done in dataloader after weighted mask
        # Just handle channel dimension
        ...
        # Classification models that need RGB input (check with startswith for variants like resnet18, resnet50, etc.)
        classification_rgb_prefixes = ["resnet", "efficientnet", "convnext", "vit", "pvt"]

        is_nnunet = model_name.startswith("nnunet")
        is_classification_rgb = any(model_name.startswith(prefix) for prefix in classification_rgb_prefixes)
        needs_rgb = (model_name in imagenet_models and not is_nnunet) or is_classification_rgb

        if needs_rgb:
            # Convert to RGB by repeating channel (but NO normalization yet)
        ...
        # For images: DO NOT normalize here
        patch = patch[np.newaxis, :, :, :]
