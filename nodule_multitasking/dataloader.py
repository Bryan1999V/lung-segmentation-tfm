"""Nodule Segmentation and Classification DataLoader Module.

This module provides data loading functionality for multi-task learning:
    - Segmentation Task: Binary lung nodule segmentation
        * Class 0: No pulmonary nodule (background)
        * Class 1: Pulmonary nodule (benign or malignant)
    - Classification Task: Nodule malignancy classification
        * Class 0: Benign nodule
        * Class 1: Malignant nodule

The dataset loads CT patches, segmentation masks, and malignancy labels simultaneously.
"""
...
class CTNoduleSegmentationAndClassificationDataset(Dataset):
    """Multi-Task Dataset for Nodule Segmentation and Malignancy Classification.
    
    This dataset loads CT nodule patches and provides both:
        1. Binary segmentation masks (0: background, 1: nodule)
        2. Binary malignancy labels (0: benign, 1: malignant)
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        """Get a single nodule patch with segmentation mask and classification label.
        ...
        Returns:
            Dictionary containing:
                ...
                - label: Binary malignancy label (0: benign, 1: malignant)
    ...
    def __repr__(self) -> str:
        return (
            f"CTNoduleSegmentationAndClassificationDataset(\n"
            f"  samples={len(self)},\n"
            f"  mode={self.mode},\n"
...
def get_nodule_segmentation_and_classification_data_loader(
    csv_name: str,
    mode: str = "2D",
    workers: int = 0,
    batch_size: int = 16,
    ...
    """Create a DataLoader for multi-task nodule segmentation and classification.
    ...
    Returns:
        DataLoader configured for multi-task learning
    """
    ...
    logging.info(
        f"Multi-Task DataLoader [{csv_name}]: {len(df)} samples - "
    ...
    # Create dataset
    dataset = CTNoduleSegmentationAndClassificationDataset(
        data_dir=data_dir,
        masks_dir=masks_dir,
        dataset=df,
        translations=translations,
    ...
    dataloader = DataLoader(
        ...
        shuffle=shuffle if sampler is None else False,  # Shuffle only if not using sampler
        num_workers=workers,
