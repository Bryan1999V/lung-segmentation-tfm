...
from torch.optim import Adam  # Adam optimizer for training neural networks
from torch.optim.lr_scheduler import ReduceLROnPlateau  # Scheduler to reduce learning rate on plateau
...
class ChestCTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)  # Load the image as grayscale
        mask = cv2.imread(mask_path)[:, :, 0]  # Extract the blue channel and treat it as grayscale
        ...
        if self.transform:
            ...
            image = augmented["image"].float()  # Transformed image
            mask = augmented["mask"]  # Transformed mask

        # Change mask dimensions to [channels, height, width] for compatibility with PyTorch
        mask = mask[None, :, :].float()  # Add channel dimension at the beginning and convert to float
...
class Config:
    ...
    data_csv = "/workspace/lung-segmentation-tfm/resources/csv/train.csv"  # Path to the CSV file containing image and mask file names
    # images_dir = "/root/data/images/images"  # Directory containing images
    # masks_dir = "/root/data/masks/masks"  # Directory containing masks
    images_dir = "/root/data/train"  # Directory containing images
    masks_dir = "/root/data/train_masks"  # Directory containing masks
...
def prepare_dataloader(dataset, phase):
    """
...
# Prepare dataset
dataset = data.CTLungDataset(config.images_dir, config.masks_dir)  # Create dataset instance
# dataset = ChestCTDataset(
#     df=data_df,
#     images_dir=config.images_dir,
#     masks_dir=config.masks_dir,
#     transform=A.Compose([ToTensorV2()]),  # Apply transformations based on phase
# )

# Split dataset into training, validation, and test sets
train_set, test_set = train_test_split(
    dataset,
    test_size=config.test_size,  # Proportion of data for validation
    random_state=config.seed,  # Seed for reproducibility
)

train_set, val_set = train_test_split(
    train_set,
    test_size=config.val_size,  # Proportion of data for validation
    random_state=config.seed,  # Seed for reproducibility
)


# Prepare DataLoaders for each dataset split
train_loader = prepare_dataloader(train_set, "train")  # Training DataLoader
val_loader = prepare_dataloader(val_set, "val")  # Validation DataLoader
test_loader = prepare_dataloader(test_set, "test")  # Test DataLoader
...
if TRAIN:
    ...
    model_efficientUnet = smp.Unet(
        ...
        in_channels=1,  # Number of input channels (RGB)
        classes=1,  # Number of output channels (number of classes in segmentation task)
    ...
    summary(model_efficientUnet, input_size=(config.batch_size, 1, 512, 512))
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                ...
                "/workspace/lung-segmentation-tfm/resources/models/best_model_efficient_Unet.pth",
            )
    ...
    model_unet = model.UNet(in_channels=1, n_classes=1)  # U-Net with 3 input channels (RGB) and 3 output classes
    ...
    summary(model_unet, input_size=(config.batch_size, 1, 512, 512))
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model_unet.state_dict(), "/workspace/lung-segmentation-tfm/resources/models/best_model_unet.pth")
            print("Saved Best Model")
    ...
    model_unet_nb = UNet(n_channels=1, n_classes=1, bilinear=False)  # U-Net with 3 input channels and 3 output classes
    model_unet_nb = model_unet_nb.to(config.device)  # Move the model to the configured device (CPU/GPU)

    # Display a summary of the model, including input size and number of parameters
    summary(model_unet_nb, input_size=(config.batch_size, 1, 512, 512))
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                ...
                "/workspace/lung-segmentation-tfm/resources/models/best_model_unet_nb.pth",
            )
            print("Saved Best Model")
...
model_efficientUnet = smp.Unet(
    ...
    encoder_weights="imagenet",  # Use pre-trained weights for encoder
    in_channels=1,  # Number of input channels (RGB)
    classes=1,  # Number of output channels (classes)
...
model_unet = model.UNet(in_channels=1, n_classes=1)  # U-Net with 3 input channels and 3 output classes
...
model_unet_nb = UNet(n_channels=1, n_classes=1, bilinear=False)  # U-Net with transposed convolution
...
model_efficientUnet.load_state_dict(
    torch.load("/workspace/lung-segmentation-tfm/resources/models/best_model_efficient_Unet.pth")
)
model_efficientUnet.eval()  # Set the model to evaluation mode

# Load the best saved weights for the custom U-Net model
model_unet.load_state_dict(torch.load("/workspace/lung-segmentation-tfm/resources/models/best_model_unet.pth"))
...
model_unet_nb.load_state_dict(torch.load("/workspace/lung-segmentation-tfm/resources/models/best_model_unet_nb.pth"))
...
if TRAIN:
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(dice_scores_efficientUnet, label="Validation Dice EfficientUnet", color="green", marker="o")
    plt.plot(dice_scores_unet, label="Validation Dice Unet", color="blue", marker="o")
    plt.plot(dice_scores_unet_nb, label="Validation Dice Unet notebook", color="red", marker="o")
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(iou_scores_efficientUnet, label="Validation Jaccard Index EfficientUnet", color="green", marker="o")
    plt.plot(iou_scores_unet, label="Validation Jaccard Index Unet", color="blue", marker="o")
    plt.plot(iou_scores_unet_nb, label="Validation Jaccard Index Unet notebook", color="red", marker="o")
...
def visualize_predictions(model, dataset, index, model_name: str):
    ...
    mask_np = mask.permute(1, 2, 0).cpu().numpy()  # Shape: (C, H, W)
    masks_np = np.clip(mask_np, 0, 1)  # Ensure mask values are between 0 and 1
    ...
    with torch.no_grad():
        ...
        pred_mask = (output > 0.5).float()[0].permute(1, 2, 0).cpu().numpy()  # Shape: (C, H, W)
        pred_mask = np.clip(pred_mask, 0, 1)  # Ensure prediction values are between 0 and 1
    ...
    # gt_image = np.clip(gt_image + alpha * mask_np, 0, 1)

    # Overlay Predicted Masks on the original image
    # pred_image = image_np.copy()
    # pred_image = np.clip(pred_image + alpha * pred_mask, 0, 1)
    ...
    # Save the figure
    plt.savefig(
        f"/workspace/lung-segmentation-tfm/resources/models/{model_name}_predictions_{index}.png", bbox_inches="tight"
    )
